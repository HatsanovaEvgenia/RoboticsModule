...
1. \( a*i \) — расстояние вдоль \( x_i \) от точки \( A_i \) до пересечения осей \( x_i \) и \( z*{i-1} \);
2. \( d*i \) — расстояние вдоль \( z*{i-1} \) от точки \( A*{i-1} \) до пересечения осей \( x_i \) и \( z*{i-1} \). Параметр \( d_i \) переменный, если сочленение \( i \) поступательное;
3. \( \alpha*i \) — угол между \( z*{i-1} \) и \( z_i \), измеренный вокруг оси \( x_i \);
4. \( \theta*i \) — угол между \( x*{i-1} \) и \( x*i \), измеренный вокруг оси \( z*{i-1} \). Параметр \( \theta_i \) переменный, если сочленение \( i \) вращательное.
...
$$ A*{i} = R^{z*{i-1}}_{\Theta_i} \* T^{z_{i-1}}_{d_i} \* T^{x_{i}}_{a_i} \* R^{x_{i}}\_{\alpha_i}$$
...
T =
\begin{bmatrix}
\cos q_1 \cos(q_2 + q_3) & -\cos q_1 \sin(q_2 + q_3) & \sin q_1 & \left( l_2 \cos q_2 + l_3 \cos(q_2 + q_3) \right) \cos q_1 \\
\sin q_1 \cos(q_2 + q_3) & -\sin q_1 \sin(q_2 + q_3) & -\cos q_1 & \left( l_2 \cos q_2 + l_3 \cos(q_2 + q_3) \right) \sin q_1 \\
