...
**Прямая задача кинематики** отвечает на вопрос: где будет находится и как будет ориентирован схват или рабочий орган манипулятора, при заданных значениях  обобщенных координат.
...
Осью $z_i$ *поступательной пары* является какая-либо из прямых, параллельная направляющей данной поступательной пары. 
...
$$ A_{i} = R^{z_{i-1}}_{\Theta_i} * T^{z_{i-1}}_{d_i} * T^{x_{i}}_{a_i} * R^{x_{i}}_{\alpha_i}$$
$$ A_i = 
\begin{bmatrix}
